...
| 2 | Hohe Auflösung, getrennte Ausgabe, **Skalierer** | weitgehend erledigt | `internal_scale` 1–12 und `output_resolution` getrennt gemessen; randloses Vollbild 1920x1080 und 3440x1440 ([07](07-ANZEIGE.md)); interner Faktor 6 rendert 3840x2688 = 10,3 MPixel, also mehr als 4K in beiden Richtungen; neun Skalierer-Kerne als Namen bedienbar, zwei davon erstmals erreichbar ([18](18-SKALIERER.md)) | Bildwirkung aller neun Kerne am Fenster belegt (Xvfb/xwd, [18](18-SKALIERER.md)), ihre Geschwindigkeit auf einer Grafikkarte nicht; 3840x2160 als Mitschnitt gefahren ([18](18-SKALIERER.md)); Schärfungsstufe fehlt; HiDPI, Mehrmonitor, Launcher-Bedienung |
...
| WP14 | Framerate-Umsetzung | **Schritte 1 bis 5 von 6 gebaut und gemessen** | Der GX-Strom jedes Bildes läuft ein zweites Mal (`MODERNGEKKO_GX_DRYRUN=1\|2\|3`), in Schritt 2 in einen Schatten-EFB, in Schritt 3 mit interpolierten XF-Matrizen. Bildgrenze berichtigt: die XFB-Kopie, nicht das Präsentieren (der EFB beim Präsentieren zeigte das nächste Bild ohne HUD). Danach Schatten und Bild in der Dateiauswahl bis auf die Bewegung gleich (0,2–2 % Pixel); Zwischenbild in der Dateiauswahl zu 75–77 % zwischen den Nachbarn, Mario plausibel, START-Schriftzug an den Kanten außerhalb ([20](20-VARIABLE-BILDRATE.md)). Ein Absturz beim Beenden bis zum Grund verfolgt (valgrind: Heap-Überlauf durch einen latenten Dolphin-Fehler in `InvalidateXFRange`) und behoben. Schritt 4 (Zwischenbild an Stelle des XFB, `MODERNGEKKO_GX_DRYRUN_PRESENT=1`): im Bildmitschnitt liegt das präsentierte Bild beim Aufblenden des Titels zu 99,5 % zwischen seinen Nachbarn (je 14 % Abstand zu A und B); 951 von 969 Bildern ersetzt. Das Spiel läuft durchgehend mit 30 Bildern je Sekunde, jedes Bild zweimal präsentiert. Das Zwischenbild nimmt denselben XFB-Kopierweg wie das echte Bild (geprüft). Spielszenen erreicht (`game-airstrip.json`, `game-water.json`): am Flugplatz 99 % im Intervall, beim Schwimmen mit Kamerafahrt 85–93 %, Wasser und Wellen bleiben auf Stand B (nicht in Matrizen). Zweikernbetrieb läuft mit dem statischen Kern und dem Trockenlauf, bringt auf Null-Grafik nichts (+0,5 %). Schritt 5 gebaut (`MODERNGEKKO_GX_DRYRUN_PRESENT=m`, 2m Präsentierungen je Spielbild aus einem CoreTiming-Ereignis): bei m = 2 wandern die Abstände der drei Zwischenbilder mit t (Titel: 3,4→16,2 % zu A, 15,5→2,7 % zu B). Offen: Bildfolge und Kosten auf echter Hardware (6) |
