...
| [18-SKALIERER.md](18-SKALIERER.md) | Ausgabe-Skalierer als Produktfunktion: neun Kerne, zwei davon erstmals erreichbar. Interner Faktor 6 rendert 3840×2688, also mehr als 4K. Bildwirkung aller neun Kerne am Fenster belegt (`tools/diagnostics/window_capture.py`, Xvfb + xwd); Geschwindigkeit auf einer Grafikkarte offen |
...
| [20-VARIABLE-BILDRATE.md](20-VARIABLE-BILDRATE.md) | Plan für WP14 in sechs Schritten, beide Vorabprüfungen erledigt. **Schritte 1 bis 5 gebaut und gemessen** (`recompcore-gx-trockenlauf.patch`, `MODERNGEKKO_GX_DRYRUN=1|2|3`, dazu `MODERNGEKKO_GX_DRYRUN_PRESENT=1`): der Strom läuft ein zweites Mal, in Schritt 2 in einen Schatten-EFB, in Schritt 3 mit interpolierten Matrizen, in Schritt 4 tritt das Zwischenbild bei 30-Hz-Bildern an die Stelle des XFB. Bildgrenze ist die XFB-Kopie (berichtigt am 16.09.). Schritt 4 im Mitschnitt belegt (Titel-Aufblende: 99,5 % im Intervall). Zwischenbild nimmt denselben XFB-Kopierweg wie das echte Bild. Spielszenen erreicht (`tools/acceptance/fixtures/game-airstrip.json`, `game-water.json`, Schritte ≤ 300 Bilder): Flugplatz 99 % im Intervall, Schwimmen mit Kamerafahrt 85–93 %. Zweikern läuft, bringt auf Null nichts. Schritt 5 gebaut und im Mitschnitt belegt (`MODERNGEKKO_GX_DRYRUN_PRESENT=2`: drei Zwischenbilder je Spielbild, Abstände wandern mit t). Offen: Schritt 6 auf dem Windows-Rechner |
...
**Modulbau:** Der Patch `dolrecomp-dcbf-bleibt-im-modul.patch` ändert den
erzeugten Code; ein vorhandener Modul-Cache passt nicht mehr und
`moderngekko-port build` legt ein neues Verzeichnis an (der Cache-Schlüssel
enthält die Prüfsumme des `dolrecomp`-Binärs). Rund 60 Minuten.
